...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hero__logo{height:96px;width:auto;max-width:100%;margin-bottom:26px}
    .hero h1{
    ...
    @media (max-width:600px){
      ...
      .hero__logo{height:auto;max-height:70px;max-width:88%;margin-bottom:20px}
    ...
    /* ==================================================================
       CAPA DE MOVIMIENTO
       Aplicada con la skill "adding-motion-and-effects" (Skiper UI).
       Presupuesto respetado: micro-interacciones libres + UNA transición
       de entrada + UN efecto de lucimiento (el reveal de las tarjetas).
       Curva estándar .4,0,.2,1 y duraciones dentro de las bandas de la skill.
       ================================================================== */
    :root{ --curva:cubic-bezier(.4,0,.2,1); --mi:.22s; --estado:.32s; }

    /* --- micro-interacciones: hover Y su equivalente de teclado ---------
       La skill audita esto: un efecto solo en :hover deja fuera a quien
       navega con tabulador. Cada regla va duplicada con :focus-visible. */
    .tarjeta{transition:transform var(--mi) var(--curva), box-shadow var(--mi) var(--curva)}
    .tarjeta:hover,
    .tarjeta:focus-visible{transform:translateY(-4px);box-shadow:var(--sombra-fuerte)}
    .tarjeta:focus-visible{outline:3px solid var(--teja);outline-offset:3px}
    .tarjeta__flecha{transition:transform var(--mi) var(--curva)}
    .tarjeta:hover .tarjeta__flecha,
    .tarjeta:focus-visible .tarjeta__flecha{transform:translateX(4px)}
    .tarjeta:active{transform:translateY(-1px)}

    .nav__links a{transition:background var(--mi) var(--curva), color var(--mi) var(--curva)}
    .nav__links a:focus-visible{background:var(--beige);color:var(--marron);
      outline:2px solid var(--teja);outline-offset:2px}

    .btn{transition:background var(--mi) var(--curva), color var(--mi) var(--curva),
      transform var(--mi) var(--curva)}
    .btn--primario:focus-visible{background:var(--marron);transform:translateY(-2px)}
    .btn--fantasma:focus-visible{background:var(--teja);color:#fff}
    .btn:focus-visible{outline:3px solid var(--marron);outline-offset:3px}
    .btn:active{transform:translateY(0)}

    .chip-eco{transition:transform var(--mi) var(--curva), box-shadow var(--mi) var(--curva)}
    .chip-eco:hover{transform:translateY(-2px);box-shadow:var(--sombra-fuerte)}

    /* --- la transición de entrada (una sola, al cargar) ---------------- */
    @keyframes mc-entra{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}
    .hero__logo,.hero h1,.hero .tagline,.hero__frase,.hero__cta{
      animation:mc-entra .5s var(--curva) both;
    }
    .hero h1{animation-delay:.07s}
    .hero .tagline{animation-delay:.14s}
